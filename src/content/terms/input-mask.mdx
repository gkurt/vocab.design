...
status: published
definition: >-
  A field that imposes a fixed shape as you type, inserting the slashes, spaces or
  brackets a phone number or card number needs.
aliases:
  - name: masked input
    source: mantine
  - name: format mask
...
The gentler alternative is to accept anything and clean it up quietly: let the field
