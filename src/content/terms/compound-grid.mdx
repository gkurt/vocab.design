...
status: published
definition: >-
  Two or more grids of different column counts overlaid in one layout, so a page can carry
  several rhythms while still sharing one structure.
aliases:
  - name: compound grids
    source: community
  - name: overlaid grids
    source: community
relations:
  contrastWith: [broken-grid, modular-grid]
sources:
