...
status: published
definition: >-
  A control shaped like a sliding track that turns one setting on or off and
  applies the change the moment it is flipped, with no separate save step.
aliases:
  - name: toggle switch
    source: primer
  - name: toggle
  ...
  - name: on off switch
    source: community
relations:
  contrastWith: [checkbox, quick-settings-tile, toggle-button]
implementations:
  - system: aria-apg
    name: Switch
