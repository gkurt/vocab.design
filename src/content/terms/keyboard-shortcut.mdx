...
status: published
definition: >-
  A key or key combination that runs a command directly, giving practised users a
  path that skips the menus and the pointer entirely.
aliases:
  - name: hotkey
  - name: accelerator
  - name: key binding
...
tags: [keyboard]
relations:
  contrastWith: [kbd, command-palette, character-key-shortcuts, chorded-shortcut]
  seeAlso: [keyboard-shortcuts-dialog, key-repeat]
implementations:
  - system: aria-apg
