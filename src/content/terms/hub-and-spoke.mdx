...
status: published
definition: >-
  A navigation shape where every task is reached from one home screen and returns to it,
  with no way to move sideways between tasks.
aliases:
  ...
  - name: home link
    source: ui-patterns
relations:
  contrastWith: [global-navigation, drill-down-navigation]
implementations: []
sources: []
demo: inline
