...
modified: 2026-08-21
definition: >-
  A control that commits only once its thumb is dragged the length of its track, so the
  effort of the gesture is the guard against doing it by accident.
aliases:
...
tags: [dragging, touch]
relations:
  contrastWith: [hold-to-confirm, type-to-confirm]
  variantOf: [microinteraction]
  seeAlso: [dragging-alternative]
sources:
