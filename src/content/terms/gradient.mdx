...
status: published
definition: >-
  A fill that transitions between two or more colours across a shape, defined by
  colour stops and by the space the transition is interpolated in.
aliases:
  ...
  - name: ramp
relations:
  contrastWith: [color-ramp, color-stop, conic-gradient, mesh-gradient]
  seeAlso: [aurora-ui]
implementations: []
sources:
