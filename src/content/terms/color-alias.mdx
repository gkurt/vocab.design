...
status: published
definition: >-
  A second name pointing at an existing scale so one palette can answer to several meanings,
  letting yellow serve as both warning and pending without duplication.
aliases:
  - name: aliasing
    source: radix
  - name: alias token
...
tags: [tokens]
relations:
  contrastWith: [color-token, color-role, primitive-color-token]
  seeAlso: [relative-color-syntax]
implementations:
  - system: radix
    name: Aliasing
