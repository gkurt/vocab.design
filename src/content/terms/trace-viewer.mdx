...
modified: 2026-08-26
definition: >-
  The view that lays a request's spans on one time axis, each nested under the span
  that called it, so the bar that is too long or starts too late is the finding.
aliases:
  - name: trace view
    source: community
  - name: span waterfall
...
tags: []
relations:
  contrastWith: [timeline, treegrid]
  variantOf: []
  partOf: []
  seeAlso: [diff-viewer, minimap]
implementations: []
sources:
...
status code, the retry count. A list beside a detail is what a trace viewer has in
common with a [diff viewer](/diff-viewer), and what separates them is what the
horizontal direction means: a diff aligns two texts against each other line by line,
while a trace aligns one run of work against a clock.
