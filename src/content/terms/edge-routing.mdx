...
modified: 2026-08-26
definition: >-
  How a connector gets from one node to another: a bezier sweep, a right-angled staircase, or
  a straight line, picked so the crossings stay readable.
aliases:
  ...
  - name: orthogonal routing
    source: community
relations:
  contrastWith: [graph-layout]
  partOf: [node-graph]
  seeAlso: [flowchart, node-graph, node-port]
implementations: []
sources:
