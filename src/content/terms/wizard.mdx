...
status: published
definition: >-
  A flow broken into ordered steps with back and next controls, where each step
  validates before the next one is allowed.
aliases:
  ...
  - name: setup assistant
    source: community
relations:
  contrastWith: [one-thing-per-page, tearsheet, form]
  seeAlso: [step-indicator, form]
sources:
