...
modified: 2026-08-21
definition: >-
  Rotating a card about its own axis to show a second face on the back, with the hidden
  face pre-rotated and its backface hidden so only one side is ever visible.
aliases:
  - name: flip card
    source: community
  ...
  - name: reveal the back
    source: community
relations:
  contrastWith: [flip-animation, page-curl]
  seeAlso: [morph-transition]
implementations: []
sources:
