...
status: published
definition: >-
  A self-playing loop an idle screen falls into to demonstrate itself, inherited from arcade cabinets
  and reused by kiosks and product demos.
aliases:
  - name: attract loop
    source: community
  - name: demo loop
  ...
  - name: kiosk mode demo
    source: community
relations:
  contrastWith: [float-animation]
  seeAlso: [pause-stop-hide, carousel]
implementations: []
sources:
