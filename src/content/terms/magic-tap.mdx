...
modified: 2026-08-21
definition: >-
  A screen reader gesture that fires the one action a screen is most likely about, so the
  reader does not have to hunt for the control.
aliases:
...
relations:
  contrastWith: [escape-gesture, explore-by-touch]
  seeAlso: [multi-touch]
sources:
