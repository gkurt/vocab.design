...
status: published
definition: >-
  Telling assistive technology how big a list really is and where an item sits in it, so a
  windowed or paginated list still announces "item 6 of 16".
aliases:
  - name: aria-setsize
    source: aria
  - name: aria-posinset
  ...
  - name: virtual list count
    source: community
relations:
  contrastWith: [list-virtualization, pagination]
  seeAlso: [select-all-across-pages]
implementations: []
sources:
