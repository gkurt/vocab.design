...
status: published
definition: >-
  Path based or multi finger gestures such as pinch, swipe, and drag along a track, which need a
  single pointer equivalent to stay operable.
aliases:
  ...
  - name: single pointer alternative
    source: community
relations:
  contrastWith: [multi-touch, motion-actuation, dragging-alternative]
  seeAlso: [mouse-gesture]
implementations: []
sources:
