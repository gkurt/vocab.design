...
status: published
definition: >-
  Borrowing the camera's vocabulary: dolly moves the viewer through the scene, zoom scales the scene
  itself, and the interface reads as travelling rather than rearranging.
aliases:
  - name: vertigo effect
    source: community
  - name: camera move
  ...
  - name: truck and pan
    source: community
relations:
  contrastWith: [parallax, ken-burns-effect]
  seeAlso: [zoom-transition]
implementations: []
sources:
