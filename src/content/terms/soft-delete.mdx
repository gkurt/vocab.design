...
status: published
definition: >-
  Removing an item from view but keeping it recoverable in a trash or archive for a
  period, so deletion is reversible without a confirmation step.
aliases:
  - name: trash
    source: community
  - name: recycle bin
  ...
  - name: recently deleted
    source: hig
relations:
  contrastWith: [type-to-confirm, confirmation-dialog, undo]
  seeAlso: [save-for-later, strikethrough]
implementations: []
sources:
