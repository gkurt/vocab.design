...
modified: 2026-08-21
definition: >-
  A viewport width at which a layout is allowed to change shape, defined once so every
  screen in a system reflows at the same places.
aliases:
  ...
  - name: breakpoints
    source: bootstrap
relations:
  contrastWith: [container-query, window-size-class, ram-technique]
  seeAlso: [mobile-first, content-choreography]
sources:
