...
status: published
definition: >-
  Visible stutter in motion caused by frames arriving late or not at all, so an animation
  that should be continuous reads as juddering.
aliases:
  - name: dropped frames
    source: community
  - name: jitter
...
one has its own name. Layout thrashing is the loop where script writes a style, reads a
...
`opacity` can be handled by the compositor, which keeps drawing them at the display's
