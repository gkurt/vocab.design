...
status: published
definition: >-
  A slider with two handles on one track that sets a lower and an upper bound, so
  the selection is an interval rather than a point.
aliases:
...
tags: [forms]
relations:
  contrastWith: [slider, track, date-range-picker]
implementations:
  - system: aria-apg
