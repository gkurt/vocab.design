...
modified: 2026-08-21
definition: >-
  The set of colours a space or a display can actually produce, which is why a value that
  is legal in one notation can be impossible on a given screen.
aliases:
  - name: gamut
    source: community
  - name: out of gamut
  ...
  - name: color-gamut media query
    source: css
relations:
  contrastWith: [gamut-mapping, color-space, wide-gamut]
  seeAlso: [cmyk]
implementations: []
sources:
