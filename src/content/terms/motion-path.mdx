...
status: published
definition: >-
  A defined path an element travels along, with its distance and rotation along
  that path animated instead of its x and y positions.
aliases:
  - name: offset-path
    source: css
...
guidance asks elements that move both across and down to travel an arc rather
