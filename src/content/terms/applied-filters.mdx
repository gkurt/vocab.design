...
status: published
definition: >-
  A row of removable tokens above the results showing every filter currently in
  force, with a control to clear them all at once.
aliases:
  - name: filter chips
    source: material
  - name: filter pills
...
tags: [search]
relations:
  contrastWith: [faceted-filter, faceted-search]
  seeAlso: [chip, bulk-actions]
implementations: []
sources:
