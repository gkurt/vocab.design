/**
 * Steampunk specimen: a brass control panel. A riveted plate carries an engraved name
 * plate, a pressure gauge with a painted cream face and a red needle, two levers in
 * slotted tracks, and a valve wheel, all in Victorian serif on aged cream.
 *
 * The panel is the subject; the caption below it is scenery. Brass, verdigris, and the
 * gauge face are stated inline because the material is this term's own claim. The brushed
 * grain comes from the kit's texture primitive, which is what it is for.
 *
 * Static: the needle rests where it rests, since a specimen that drove the gauge would be
 * demonstrating a meter rather than a style.
 */
const SERIF = "'Baskerville', 'Times New Roman', Georgia, serif";
const BRASS = 'linear-gradient(155deg, #e3c684 0%, #b08a45 34%, #8a6a2e 62%, #6a5023 100%)';
const DARK = '#2c2114';
const CREAM = '#efe4c8';
const RED = '#8f2318';
...
export function mount(root: HTMLElement): void {
  const rivet = (pos: string) =>
    `<span aria-hidden="true" style="position: absolute; ${pos}; width: 9px; height: 9px; border-radius: 50%;
            background: radial-gradient(circle at 34% 32%, #f2dea8, #7a5c26 72%); box-shadow: 0 1px 1px rgb(0 0 0 / 0.5)"></span>`;
  const rivets = [
    'left: 8px; top: 8px',
    'right: 8px; top: 8px',
    'left: 8px; bottom: 8px',
    'right: 8px; bottom: 8px',
    'left: 50%; top: 8px; margin-left: -4.5px',
    'left: 50%; bottom: 8px; margin-left: -4.5px',
    'left: 8px; top: 50%; margin-top: -4.5px',
    'right: 8px; top: 50%; margin-top: -4.5px',
  ]
    .map(rivet)
    .join('');

  const lever = (left: number, angle: number, top: number) => `
    <span aria-hidden="true" style="position: absolute; left: ${left}px; top: 56px; width: 16px; height: 92px; border-radius: 9px;
          background: #241a0f; box-shadow: inset 0 2px 4px rgb(0 0 0 / 0.7), 0 1px 0 rgb(240 220 160 / 0.35)"></span>
    <span aria-hidden="true" style="position: absolute; left: ${left + 3}px; top: ${top}px; width: 10px; height: 44px;
  ...
  const spoke = (angle: number) =>
    `<span style="position: absolute; left: 50%; top: 50%; width: 46px; height: 5px; margin: -2.5px 0 0 -23px;
            background-image: ${BRASS}; transform: rotate(${angle}deg)"></span>`;
    ...
    <div class="sp-app" style="gap: 10px">
      <div data-part="panel" data-subject class="sp-texture"
           style="position: relative; width: 258px; height: 244px; --sp-texture-base: #8a6c35; --sp-texture-angle: 90deg;
                  border: 3px solid #5d461e; border-radius: 6px; font-family: ${SERIF}; overflow: hidden;
                  box-shadow: inset 0 0 26px rgb(44 33 20 / 0.65)">

        ${rivets}

        <span data-part="plate"
              style="position: absolute; top: 13px; left: 50%; translate: -50% 0; padding: 3px 14px; background: ${CREAM};
                     border: 1px solid #5d461e; border-radius: 2px; color: ${DARK}; font-size: 11px; font-weight: 700;
        ...
        <span data-part="gauge" aria-hidden="true"
              style="position: absolute; left: 20px; top: 50px; width: 104px; height: 104px; padding: 7px; border-radius: 50%;
                     background-image: ${BRASS}; box-shadow: 0 3px 6px rgb(0 0 0 / 0.5), inset 0 1px 0 rgb(255 245 210 / 0.5)">
          <span style="position: relative; display: block; width: 100%; height: 100%; border-radius: 50%;
            ...
            <span style="position: absolute; inset: 4px; border-radius: 50%;
                         background-image: repeating-conic-gradient(from 212deg, ${DARK} 0 1.2deg, transparent 1.2deg 17deg)"></span>
            ...
            <span data-part="needle"
                  style="position: absolute; left: 50%; top: 50%; width: 3px; height: 33px; background: ${RED};
            ...
            <span style="position: absolute; left: 50%; top: 50%; width: 12px; height: 12px; translate: -50% -50%;
                         border-radius: 50%; background-image: ${BRASS}; box-shadow: 0 1px 2px rgb(0 0 0 / 0.5)"></span>
          </span>
        </span>
        ...
        <span data-part="wheel" aria-hidden="true"
              style="position: absolute; right: 18px; top: 62px; width: 62px; height: 62px; border-radius: 50%;
                     border: 6px solid transparent; background-image: ${BRASS}; background-clip: border-box;
        ...
        <span data-part="foot"
              style="position: absolute; left: 20px; bottom: 20px; padding: 2px 9px; background: rgb(239 228 200 / 0.9);
                     border: 1px solid #5d461e; color: ${DARK}; font-size: 9px; letter-spacing: 0.18em;
