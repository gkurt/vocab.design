 ...
 * every state and needs no `data-pose`. The sentence, the picker and the verdict are scenery
 * (SPEC §5).
 *
 * A caption under the transcript used to explain each state ("No hint, so the voice picks the
 * commoner reading and the sentence becomes an instruction about following pipes. Nothing on
 * the page looks wrong."). That is the article's job, and the strip already carries a verdict
 * that changes with the switch, so the caption is gone rather than moved.
 *
