...
type Note = 'rest' | 'added' | 'browse' | 'dismissed' | 'checkout';

const NOTE: Record<Note, string> = {
  rest: 'Nothing added yet. The basket already holds a line, which is why an added line has to be marked as the new one.',
  added: 'The add is confirmed where it happened: the new line marked, a subtotal, and both routes out. The product page never left.',
  browse: 'Opened from the cart icon with nothing just added: the same panel, doing the other half of its job.',
  dismissed: 'Dismissed by the reader, not by a timer. The page is exactly where it was, and the count carries the basket.',
  checkout: 'Checkout is the one route out that does leave this page, which is why it is a choice rather than the only door.',
};

const thumb = '<span style="flex: 0 0 auto; width: 28px; height: 28px; border-radius: 6px; background: var(--sp-sunken)"></span>';

/**
 * Mini cart specimen: a product page, an add to basket, and the compact basket panel that
 * confirms it without leaving the page. The panel opens on the add and again from the cart
 * icon, and closes on its own close control or on Keep shopping, so every route in and out
 * is explicit rather than a toggle (SPEC §8).
 *
 * The subject is the panel, which is what the term names: the product page behind it, the
 * topbar, the cart trigger and the note line are scenery (SPEC §5). It is hidden at mount,
 * so identify summons the add that opens it (SPEC §6), and it is a mini cart in every state
 * it is on stage, so no `data-pose` is needed.
 *
 * Built as a popover hanging off the cart icon rather than as an edge-docked drawer, since
 * the position is the one thing about this pattern that varies by site. The line area keeps
 * room for both lines from mount, so the added line does not resize the panel, and the
 * closed panel is laid over the page rather than in it, so opening moves nothing (SPEC §5).
 */
export function mount(root: HTMLElement): void {
    ...
    <div class="sp-app">
      <div class="sp-frame sp-frame--wide" style="width: 476px; height: 272px">
        <div class="sp-topbar sp-context" style="padding: 6px 12px">
          ...
          >
            ${icon('inbox')}<span data-part="cart-count" style="font-size: 12px; font-variant-numeric: tabular-nums">2</span>
          </button>
        </div>
        ...
        >
          <div class="sp-row sp-row--between" style="gap: 8px">
            ...
            <button class="sp-icon-button" data-part="close" type="button" aria-label="Close" style="flex: 0 0 auto; width: 24px; height: 24px">${icon('close')}</button>
          </div>
      ...
      <span class="sp-text sp-context" data-stage-verdict data-part="note" style="width: 452px; height: 32px; font-size: 11px; line-height: 1.35">${NOTE.rest}</span>
    </div>
  `;

  const panel = part(root, 'panel');
  const cart = part(root, 'cart');
  const count = part(root, 'cart-count');
  const newLine = part(root, 'new-line');
  const subtotal = part(root, 'subtotal');
  const subtotalValue = part(root, 'subtotal-value');
  ...
  part(root, 'add').addEventListener('click', () => {
    ...
    newLine.dataset.state = 'added';
    cart.dataset.count = '3';
    count.textContent = '3';
    subtotal.dataset.total = '52.00';
    subtotalValue.textContent = '52.00';
  ...
  cart.addEventListener('click', () => {
    flag(panel, 'data-open', true);
    say(added ? 'added' : 'browse');
  });

  for (const name of ['close', 'keep']) {
  ...
  part(root, 'checkout').addEventListener('click', () => say('checkout'));
}
