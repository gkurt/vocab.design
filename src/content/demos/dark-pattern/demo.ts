 ...
 * deceptive version is the resting state, because the term names the manipulation:
 * pointing identify at the fair version would be pointing it at a different word.
 *
 * A label sat over the block reading "Delivery protection upsell (the mistake)",
 * which was the author scoring the scene from inside the checkout, and which the
 * strip's verdict already says properly. It is gone; the block's own heading names
 * the step, as a checkout's would.
 *
 * The subject is the choice block, not the checkout around it. The order summary,
 * the verdict line, and the state control are scenery (SPEC §5). The
