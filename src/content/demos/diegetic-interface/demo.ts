...
const NOTE = {
  world:
    'Diegetic: the ammunition is printed on the weapon and the health is a display on the wrist, so the character can read both. Angle, shadow and perspective apply to them.',
  hud: 'Non-diegetic: the same two facts in screen space, flat, square to the frame, and visible to the player alone. The world does not contain them.',
};

/**
 * Diegetic interface specimen: one game scene carrying the same two facts (ammunition and
 * health) two ways. In world, they are surfaces the character owns, rotated and skewed with
 * the objects they belong to. On screen, they are a flat HUD in the frame's own coordinates.
 *
 * The subject is the in-world readout on the weapon, the narrowest element the term names
 * (SPEC §5): the counter itself, not the weapon it is printed on and not the scene. It is
 * absent in the non-diegetic state rather than dishonest there, so identify summons it
 * instead of posing a lie, and no `data-pose` is needed. The wrist display is a second
 * instance of the same idea, left unmarked because exactly one subject may exist.
 *
 * The scene's geometry is identical in both states: the weapon, the arm and the wrist device
 * never move, and only where the numbers live changes, so switching shifts nothing (SPEC §5).
 * The literal colours are the term's own paint: a game world is not drawn in kit neutrals.
 */
    ...
    <div class="sp-app">
      <div class="sp-frame sp-frame--wide" style="width: 460px; height: 262px">
        <div class="sp-topbar sp-context" style="padding: 7px 12px">
          ...
          <sp-segmented class="sp-segmented" data-part="mode" data-value="world" style="flex: 0 0 auto">
      ...
      <span class="sp-text sp-context" data-part="note" style="width: 452px; height: 30px; flex: 0 0 auto; font-size: 11px; line-height: 1.35">${NOTE.world}</span>
    </div>
  `;

  const scene = part(root, 'scene');
  const readout = part(root, 'readout');
  const figures = part(root, 'wrist-figures');
  const hud = part(root, 'hud');
  const note = part(root, 'note');

  const render = (mode: 'world' | 'hud') => {
    scene.dataset.mode = mode;
    readout.style.display = mode === 'world' ? 'flex' : 'none';
    figures.style.display = mode === 'world' ? 'block' : 'none';
    hud.style.display = mode === 'hud' ? 'block' : 'none';
    note.textContent = NOTE[mode];
  };

  part(root, 'mode').addEventListener('change', (event) => {
    render((event as CustomEvent<string>).detail === 'hud' ? 'hud' : 'world');
  });
