...
/**
 * Tag specimen: an issue carrying three classifications. The subject is one tag,
 * not the row, since the word names the label rather than the set of them.
 *
 * The removable tag is last in the row and the row keeps its height (SPEC §5),
 * so taking one off moves nothing that stayed.
 */
export function mount(root: HTMLElement): void {
  ...
  part(root, 'tag-triage-remove').addEventListener('click', () => part(root, 'tag-triage').remove());
}
