...
// spelled out more than once in the file, and a name emitted from a helper or template that
// the demo itself reads back with `partsOf`, which proves at authoring time that the author
// expected several. A name used ONLY through `partsOf` and never aimed at by a script is
// working as intended and is not reported.
//
