var e=200,t=150,n=`
  <svg data-part="grain" aria-hidden="true" width="${e}" height="${t}"
       style="position: absolute; inset: 0; pointer-events: none; mix-blend-mode: overlay; opacity: 0.42">
    <filter id="rf-grain">
      <feTurbulence type="fractalNoise" baseFrequency="0.82" numOctaves="3" stitchTiles="stitch"/>
      <feColorMatrix type="saturate" values="0"/>
    </filter>
    <rect width="100%" height="100%" filter="url(#rf-grain)"/>
  </svg>`,r=`linear-gradient(180deg, #f6cf9b 0%, #eda269 55%, #d9744d 100%)`,i=`
  <svg viewBox="0 0 ${e} ${t}" width="${e}" height="${t}" role="presentation" style="display: block">
    <circle cx="132" cy="54" r="17" fill="#fff5d2"/>
    <path d="M0 96 44 74l38 15 32-19 42 20 44-14v40H0z" fill="#c07a53"/>
    <path d="M0 112 38 96l40 14 34-10 46 16 42-12v34H0z" fill="#8a4c39"/>
    <path d="M0 126h200v24H0z" fill="#4a2a23"/>
    <path d="M28 126v-20l6-7 6 7v20zM162 126v-15l5-6 5 6v15z" fill="#3a2019"/>
  </svg>`;function a(e,t){return`<span data-part="${e}" aria-hidden="true" style="position: absolute; inset: 0; pointer-events: none; ${t}"></span>`}function o(n,a,o){return`
    <span data-part="${n}"${a}
          style="position: relative; display: block; width: ${e}px; height: ${t}px; overflow: hidden;
                 isolation: isolate; border-radius: 3px; background-image: ${r}">
      ${i}${o}
    </span>`}function s(t,n,r){return`
    <div class="sp-stack${r?` sp-context`:``}" style="flex: 0 0 auto; width: ${e}px; gap: 6px; align-items: center">
      ${n}
      <span class="sp-label" style="color: var(--sp-ink); font-size: 12px">${t}</span>
    </div>`}function c(e){e.innerHTML=`
    <div class="sp-app" data-loop="keep" style="gap: 9px">
      <div class="sp-window" style="width: 466px; padding: 14px 16px 16px">
        <div class="sp-row" data-part="tour" style="gap: 18px; align-items: flex-start; justify-content: center">
          ${s(`Treated`,o(`treated`,` data-subject`,[a(`halation`,`mix-blend-mode: screen; background: radial-gradient(circle at 66% 36%, rgb(255 192 116 / 0.74) 0%, rgb(255 142 66 / 0.4) 19%, rgb(255 120 48 / 0.14) 36%, transparent 58%)`),a(`fade`,`background: linear-gradient(184deg, rgb(255 224 180 / 0.26), rgb(104 72 56 / 0.26))`),a(`vignette`,`background: radial-gradient(122% 104% at 50% 46%, transparent 42%, rgb(34 18 12 / 0.54) 100%)`),n,`<span data-part="dust" aria-hidden="true" style="position: absolute; inset: 0; pointer-events: none">
       <span style="position: absolute; left: 38px; top: 16px; width: 2px; height: 21px; background: rgb(255 246 226 / 0.5); rotate: 8deg"></span>
       <span style="position: absolute; left: 152px; top: 96px; width: 2px; height: 13px; background: rgb(255 246 226 / 0.42); rotate: -12deg"></span>
       <span style="position: absolute; left: 74px; top: 118px; width: 3px; height: 3px; border-radius: 50%; background: rgb(38 22 16 / 0.55)"></span>
       <span style="position: absolute; left: 108px; top: 30px; width: 2px; height: 2px; border-radius: 50%; background: rgb(38 22 16 / 0.5)"></span>
     </span>`,`<span data-part="datestamp" aria-hidden="true"
           style="position: absolute; right: 9px; bottom: 7px; font-family: ui-monospace, 'SFMono-Regular', Menlo, monospace;
                  font-size: 12px; letter-spacing: 0.06em; color: #ff8b3c; text-shadow: 0 0 7px rgb(255 139 60 / 0.85)">
       '89 08 14
     </span>`].join(``)),!1)}
          ${s(`As drawn`,o(`clean`,``,``),!0)}
        </div>
      </div>

      <p class="sp-text sp-context" data-stage-verdict data-part="caption" style="max-width: 466px; margin: 0; text-align: center">
        Grain is silver, halation is light bouncing back off the film base, and here both are drawn rather than captured.
      </p>
    </div>
  `}export{c as mount};